...
router.post("/login", function(req, res, next) {
  ...
  db.all("SELECT id, username, password_hash FROM User WHERE Email = (?)", [req.body.email], function(err, users) {
    if (err) {
      ...
      bcrypt.compare(req.body.password, password_hash, function(err, res_bcrypt) {
        if (err || !res_bcrypt){
          console.log(err.stack);
...
router.post("/register", function(req, res, next) {
  ...
  db.all("SELECT id, username, password_hash FROM User WHERE Email = (?) OR Username = (?)", [email, username], function(err, users) {
    if (err) {
      console.log(err.stack);
      res.status(500).send();
    } else if (users.length != 0) {
      res.status(200).send({ success: false, message: "User with email or username already exists"});
    } else {
